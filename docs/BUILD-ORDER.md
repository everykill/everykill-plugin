...
| 8 | Grading, batching, upload | ✅ **verified** | 3 real cyclops kills travelled detector → queue → server unaided 2026-08-25; drops and dryness verdicts intact |
...
**🟡 Status: partial — one measurement and one bug left.** The allocator is verified in play across three sessions. Settle timing was rebuilt and re-verified 2026-08-24 (see below), the **XP settle window is measured** over 392 allocations at two venues, and cannon XP is confirmed against the wiki in play. What remains: the **residual noise floor** is unmeasured, and the snakeling cross-tick mis-attribution is still open with a red test naming it. Also open: attribution runs ~0.25% high because non-combat XP earned mid-fight is allocated to the monster — FINDINGS 2026-08-21.
...
- ~~**Fix the settle timing.**~~ — **done 2026-08-24, verified in a client.** `damage()` now only accumulates; `settle()` runs on the game tick, matching what `SpecialCounterPlugin`, `LootManager` and `XpTrackerPlugin` all do. Two monsters hit on one tick now split by damage share, a path that had never executed in play. **Settle at `getTickCount() - 1`, not `getTickCount()`** — `GameTick` fires *before* that tick's hitsplats, and settling the in-progress tick strands XP whose pool is filled moments later. That regression was caught only in a client; all 49 unit tests passed through it.
- **The snakeling case is still open, and it is a different problem than it looked.** `aChipHitOnAnAddDoesNotStealTheBossesExperience` remains red on purpose. The add's damage is on tick 100 and the boss's on 101 — **separate ticks**, so tick-boundary pooling cannot help. It is a search-order question, and the "search forward before own-tick" hypothesis was **falsified** the same day when offset 0 and -1 both turned up at a second venue. Needs a fresh approach, not a reorder.
