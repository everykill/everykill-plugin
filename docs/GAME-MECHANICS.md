...
| **Ironman: another player attacking the monster destroys the drop entirely** | *"No loot will drop whatsoever if another player has attacked that monster. For some monsters, **even zero points of damage** dealt by another player will prevent the Ironman from getting any loot."* Not "they get it" and not "you can't pick it up" — **it never spawns.** Also triggered by a **goading potion**, and by the monster attacking others too much (anti-tanking). Does **not** apply when the monster was weakened by other *monsters*. Assisted kills also void Achievement Diary monster tasks | [Ironman Mode](https://oldschool.runescape.wiki/w/Ironman_Mode) | 2026-08-24 |
| **Account type varbit** | `VarbitID.IRONMAN` = **1777**. **0** main, **1** Ironman, **2** Ultimate, **3** Hardcore. Read live at point of use, not cached at login | core `HiscorePlugin.java:277`, `DailyTasksPlugin.java:212` | 2026-08-24 |
| **Group Ironman is NOT in that varbit** | Core's switch has no case for it and falls through to NORMAL. Group status comes from `client.getClanSettings(ClanID.GROUP_IRONMAN)` — the group's clan channel. A GIM therefore looks like a plain ironman or a main if you read the varbit alone | core `NameAutocompleter.java:241` | 2026-08-24 |
| **A groupmate is not an outsider** | The in-game warning reads *"you don't get loot if players outside **your group** helped you kill the monster"* — so a groupmate's damage does not void a GIM's drop. Core already models this: `GroundItem.getOwnership()` returns `OWNERSHIP_SELF` / `OWNERSHIP_GROUP` / `OWNERSHIP_OTHER` | in-game message + core `GroundItemsPlugin` | 2026-08-24 |
| Which monsters void loot on **zero** damage, and whether a splash is visible client-side | **UNVERIFIED and it matters.** A splash or blocked hit may produce no hitsplat, leaving `othersDamage` at 0 while the game has already voided the drop. If so, an ironman kill can look clean to us and be lootless in fact. Needs a live test, not an assumption | — | UNVERIFIED |
